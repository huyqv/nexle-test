...
inline fun <reified T : Any> String?.parseArray(): List<T>? {
    this ?: return null
    if (isNullOrEmpty()) {
        return null
    }
    return try {
        return gson.fromJson(StringReader(this), Array<T>::class.java).toList()
    } catch (ignore: Exception) {
        null
    }
}
...
inline fun <reified T : Any> JsonArray?.parseArray(): List<T>? {
    return this?.toString()?.parseArray()
}
...
fun JsonElement?.toMap(): Map<String, Any?>? {
    try {
        ...
        return gson.fromJson(
            this, object : TypeToken<java.util.HashMap<String?, Any?>?>() {}.type
        )
    } catch (ignore: Exception) {
        return null
    }
 ...
 */
fun String?.toJsonObject(): JsonObject? = parse()

fun String?.toJsonArray(): JsonArray? = parse()
...
inline fun <reified T : Any> JsonObject?.list(key: String): List<T>? {
    return array(key)?.parseArray()
}
